...
INSERT INTO email_templates (
  ...
  'Đặt lại mật khẩu',
  'Yêu cầu đặt lại mật khẩu - Giặt Ký',
  '<html><body><h2>Xin chào {{full_name}},</h2><p>Chúng tôi nhận được yêu cầu đặt lại mật khẩu cho tài khoản của bạn.</p><p>Vui lòng click vào liên kết dưới đây để thực hiện thay đổi mật khẩu:</p><p><a href="{{reset_link}}" style="background-color:#2563EB;color:white;padding:10px 20px;text-decoration:none;border-radius:5px;">Đặt Lại Mật Khẩu</a></p><br/><p>Nếu bạn không thực hiện yêu cầu này, vui lòng bỏ qua email.</p><p>Trân trọng,<br/>Đội ngũ Giặt Ký</p></body></html>',
  ...
  'Đặt đơn thành công',
  'Đơn hàng mới {{order_code}} đã được tiếp nhận - Giặt Ký',
  '<html><body><h2>Chào {{full_name}},</h2><p>Cảm ơn quý khách đã sử dụng dịch vụ của Giặt Ký.</p><p>Đơn hàng của quý khách đã được tạo thành công với thông tin như sau:</p><ul><li>Mã đơn hàng: <strong>{{order_code}}</strong></li><li>Cơ sở tiếp nhận: {{branch_name}}</li><li>Tổng số tiền: {{total_amount}} VNĐ</li><li>Trạng thái thanh toán: {{payment_status}}</li><li>Thời gian hẹn trả dự kiến: {{expected_return_at}}</li></ul><p>Chúng tôi sẽ thông báo cho quý khách khi đồ giặt sẵn sàng để nhận.</p><p>Trân trọng,<br/>Giặt Ký</p></body></html>',
  ...
  'Thông báo hệ thống',
  'Thông báo từ Ban quản trị - Giặt Ký',
  '<html><body><h2>Thông báo mới,</h2><p>Gửi các thành viên của Giặt Ký:</p><div style="border-left:4px solid #2563EB;padding-left:15px;margin:15px 0;"><strong>{{title}}</strong><br/><p>{{content}}</p></div><p>Trân trọng,<br/>Ban quản trị</p></body></html>',
  '["title", "content"]'::jsonb,
  'announcement',
  true
), (
  ...
  'Bảng tính lương',
  'Bảng lương tháng {{month}}/{{year}} của bạn - Giặt Ký',
  '<html><body><h2>Xin chào {{full_name}},</h2><p>Bảng lương tháng {{month}}/{{year}} của bạn đã được tính thành công:</p><ul><li>Tổng số giờ làm việc: {{total_hours}} giờ</li><li>Đơn giá lương: {{hourly_rate}} VNĐ/giờ</li><li>Tổng lương nhận được: <strong>{{total_salary}} VNĐ</strong></li><li>Trạng thái: {{status}}</li></ul><p>Vui lòng đăng nhập vào ứng dụng để kiểm tra chi tiết ca làm việc.</p><p>Trân trọng,<br/>Giặt Ký</p></body></html>',
