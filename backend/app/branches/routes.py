...
def get_branches(current_user: dict = Depends(get_current_user)):
    ...
    Admin sees all.
    Manager sees branches they manage.
    Staff sees their assigned branch.
    """
    role = current_user["role"]
    
    if role == "admin":
        response = supabase.table("branches").select("*, manager:users!manager_id(full_name)").order("created_at", desc=True).execute()
        return response.data or []
        
    elif role == "manager":
        # Get branches where manager_id equals current user id
        response = supabase.table("branches").select("*, manager:users!manager_id(full_name)").eq("manager_id", current_user["id"]).execute()
        return response.data or []
        
    elif role == "staff":
        if not current_user.get("branch_id"):
            return []
        response = supabase.table("branches").select("*, manager:users!manager_id(full_name)").eq("id", current_user["branch_id"]).execute()
        return response.data or []
...
def create_branch(payload: BranchCreate, current_user: dict = Depends(get_current_user)):
    ...
    if payload.manager_id:
        supabase.table("users").update({"branch_id": branch["id"]}).eq("id", payload.manager_id).execute()
...
def update_branch(id: str, payload: BranchUpdate, current_user: dict = Depends(get_current_user)):
    ...
    if current_user["role"] != "admin":
        ...
        branch_res = supabase.table("branches").select("manager_id").eq("id", id).execute()
        if not branch_res.data or branch_res.data[0]["manager_id"] != current_user["id"]:
            raise HTTPException(status_code=403, detail="Bạn chỉ có thể cập nhật cơ sở mình quản lý.")
